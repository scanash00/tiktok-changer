...
def main():
    ...
    print_dotted_border(f"{Fore.CYAN}Retrieving profile...{Style.RESET_ALL}")
    user = get_profile(session_id, device_id, iid)
    if user != "None":
        print_dotted_border(f"{Fore.GREEN}Your current TikTok username is: {Fore.YELLOW}{user}{Style.RESET_ALL}")
        new_username = input(f"{Fore.CYAN}Enter the new username you wish to set: {Style.RESET_ALL}")
        print_dotted_border(change_username(session_id, device_id, iid, user, new_username))
    else:
        print_dotted_border(f"{Fore.YELLOW}Attempting US version...{Style.RESET_ALL}")
        user_us = get_profile_us(session_id, device_id, iid)
        if user_us != "None":
            print_dotted_border(f"{Fore.GREEN}Your current TikTok username is: {Fore.YELLOW}{user_us}{Style.RESET_ALL}")
            new_username = input(f"{Fore.CYAN}Enter the new username you wish to set: {Style.RESET_ALL}")
            print_dotted_border(change_username_us(session_id, device_id, iid, user_us, new_username))
        else:
            print("Invalid session ID or other error.")
    print("Thanks for using this app, made by Scan")
